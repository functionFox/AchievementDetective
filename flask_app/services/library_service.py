...
def find_libraryfolders_vdf(steam_roots: Iterable[Path]) -> Path | None:
    """
...
def parse_library_paths(libraryfolders_path: Path) -> list[Path]:
    """
...
def parse_appmanifest(appmanifest_path: Path) -> dict[str, str] | None:
    """
...
def scan_steam_games() -> list[dict[str, str]]:
    ...
    libraryfolders_path = find_libraryfolders_vdf(steam_roots)
    if libraryfolders_path is None:
        return []

    library_paths = parse_library_paths(libraryfolders_path)
    ...
    for library_path in library_paths:
        ...
        for manifest_path in steamapps_path.glob("appmanifest_*.acf"):
            game = parse_appmanifest(manifest_path)
            if game is None:
                continue
